...
async function _openPatrimonioModal() {
  ...
  // assetGross = accountTotal + debtTotal (total de ativos antes de subtrair passivos)
  // Deve ser IDÊNTICO ao que o KPI usa internamente para que os totais batam
  // accountTotal já inclui: líquidas + investimentos + cartões (positivos e negativos)
  // Para exibição de barras: usamos accountTotal como base de 100%
  const assetGross = accountTotal > 0 ? accountTotal : (liquidTotal + invTotal + cardPositiveTotal);
  ...
  let content = `
    <div style="padding:20px 22px 24px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <div>
          ...
          <div style="font-size:.72rem;color:var(--muted);margin-top:2px">
            Ativos: ${dashFmt(accountTotal >= 0 ? accountTotal : 0,'BRL')}
            ${totalPassivos > 0.01 ? ` · Passivos: −${dashFmt(totalPassivos,'BRL')}` : ''}
            ${accountTotal < 0 ? ` · Saldo negativo incluso` : ''}
          </div>
        </div>
      ...
      <!-- ── ATIVOS ────────────────────────────────────────────────── -->
      <div style="font-size:.7rem;font-weight:800;text-transform:uppercase;letter-spacing:.09em;color:var(--accent);padding:4px 0 8px;border-bottom:1.5px solid var(--accent);margin-bottom:12px;display:flex;align-items:center;justify-content:space-between">
        <span>📈 ATIVOS</span>
        <span>${dashFmt(liquidTotal + invTotal + cardPositiveTotal,'BRL')}</span>
      </div>`;
    ...
    <div style="border-top:2px solid var(--border);margin-top:12px;padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">
        <div style="background:rgba(42,96,73,.08);border-radius:8px;padding:7px 10px;text-align:center">
          <div style="font-size:.62rem;color:var(--muted);margin-bottom:2px">Ativos Brutos</div>
          <div style="font-size:.82rem;font-weight:700;color:var(--accent)">+${dashFmt(liquidTotal + invTotal + cardPositiveTotal,'BRL')}</div>
        </div>
