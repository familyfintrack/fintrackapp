...
function _scCardHtml(sc) {
  ...
  const freqChip = `<span class="sc-period-chip">${esc(scFreqLabel(sc))}</span>`;
  const metaParts = [];
  ...
  return `<div class="sc-card" id="scCard-${sc.id}" data-id="${sc.id}">
    ...
    <div class="sc-card-row" onclick="toggleScCard('${sc.id}')">
      ...
      <div class="sc-card-mid">
        ...
        <div class="sc-card-meta">${freqChip}${meta ? `<span class="sc-meta-text">${meta}</span>` : ''}${catChip}${memberChips ? '<div class="sc-member-chips">' + memberChips + '</div>' : ''}</div>
      </div>
