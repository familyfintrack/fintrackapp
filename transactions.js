...
function renderTransactions(){
  ...
  function renderWithDateGroups(txList, showAcc, balMapArg) {
    ...
    let lastDate = null;
    let bandIndex = 0;
    const TODAY_STR = (typeof FinTrackDateUtils !== 'undefined' && FinTrackDateUtils.getTodayLocalISO)
      ? FinTrackDateUtils.getTodayLocalISO()
      : new Date().toISOString().slice(0,10);
    const _yd = new Date(); _yd.setDate(_yd.getDate() - 1);
    const YESTERDAY_STR = [_yd.getFullYear(), String(_yd.getMonth()+1).padStart(2,'0'), String(_yd.getDate()).padStart(2,'0')].join('-');
...
async function openTransactionModal(id=''){
  ...
  _txModalSetValue('txDate', (typeof FinTrackDateUtils !== 'undefined' && FinTrackDateUtils.getTodayLocalISO) ? FinTrackDateUtils.getTodayLocalISO() : new Date().toISOString().slice(0,10));
...
async function fetchTxCurrencyRate() {
  ...
  try {
    let txDate = document.getElementById('txDate')?.value || (typeof FinTrackDateUtils !== 'undefined' && FinTrackDateUtils.getTodayLocalISO) ? FinTrackDateUtils.getTodayLocalISO() : new Date().toISOString().slice(0,10);
