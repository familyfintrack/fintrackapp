...
async function saveDream() {
  ...
  const memberIds = typeof getFmcMultiPickerSelected === 'function'
    ...
    : [];
  // Note: family_member_ids is NOT a column in the dreams table; it lives in
  // transactions/scheduled_transactions. Exclude it from the payload to avoid
  // the PostgREST schema-cache error "Could not find the 'family_member_ids'
  // column of 'dreams'".
  const payload={family_id:fid,created_by:currentUser?.id,title:w.data.title,description:w.data.description||null,dream_type:safeType,target_amount:w.data.target_amount,currency:'BRL',target_date:w.data.target_date||null,priority:w.data.priority||1,status:w.data.status||'active',ai_generated_fields_json:w.data.ai_generated_fields_json?JSON.stringify(w.data.ai_generated_fields_json):null,updated_at:new Date().toISOString()};
  try {
    let dreamId;
